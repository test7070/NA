	...
	<head>
		...
		<script type="text/javascript">
			...
			function mainPost() {
				...
				, ['txtDime', 9, 3, 1]
				, ['txtLengthb', 9, 3, 1]
				, ['txtMount', 9, q_getPara('vcc.mountPrecision'), 1]
				, ['txtWeight', 9, q_getPara('vcc.weightPrecision'), 1]
				, ['txtTotal', 10, 0, 1]
				, ['txtC1', 10, q_getPara('vcc.mountPrecision'), 1], ['txtNotv', 10, q_getPara('vcc.mountPrecision'), 1]];
			...
			function bbsAssign() {
				for (var j = 0; j < (q_bbsCount == 0 ? 1 : q_bbsCount); j++) {
					if (!$('#btnMinus_' + j).hasClass('isAssign')) {
						...
						$('#btnCub_'+i).unbind('click');
						$('#btnCub_'+i).click(function() {
							t_IdSeq = -1;
							q_bodyId($(this).attr('id'));
							b_seq = t_IdSeq;
							if (!emp($('#txtNoa').val()) && !emp($('#txtNo2_' + b_seq).val())) {
								var t_where = "where=^^ ordeno='" + $('#txtNoa').val() + "' and no2='"+$('#txtNo2_' + b_seq).val()+"' ^^ stop=1";
								q_gt('view_cub', t_where, 0, 0, 0, "getcub", r_accy,1);
								var as = _q_appendData("view_cub", "", true);
								if (as[0] != undefined) {
									t_where = "ordeno='" + $('#txtNoa').val() + "' and no2='"+$('#txtNo2_' + b_seq).val()+"'";
									q_box("cub_na.aspx?" + r_userno + ";" + r_name + ";" + q_time + ";" + t_where, 'cub_na', "95%", "95%", '連續製令單');
								}else{
									alert('該品項未進行製令生產!!');
								}
							}
			...
			function bbsSave(as) {
				if (!as['productno'] && !as['product'] && !as['style'] && !as['spec'] && !dec(as['total'])) {
					as[bbsKey[1]] = '';
					return;
				}
				q_nowf();
		...
		<div class='dbbs' style="width: 1850px;">
			<table id="tbbs" class='tbbs' border="1" cellpadding='2' cellspacing='1'>
				<tr style='color:White; background:#003366;' >
					...
					<td align="center" style="width:85px;"><a id='lblLengthb_na_s'>長度</a></td>
					...
					<td align="center" style="width:185px;"><a id='lblMemos'> </a></td>
					...
					<td align="center" style="width:43px;"><a id='lblCub_na_s'>製令</a></td>
					<!--
				...
				<tr style='background:#cad3ff;'>
					...
					<td>
						<input class="txt c7" id="txtStyle.*" type="text" style="width: 70%;" />
					...
					<td><input class="txt num c7" id="txtRadius.*" type="text" /></td>
					<td><input class="txt num c7" id="txtWidth.*" type="text" /></td>
					<td><input class="txt num c7" id="txtDime.*" type="text" /></td>
					...
					<td align="center"><input class="btn" id="btnCub.*" type="button" value='.' style=" font-weight: bold;" /></td>
					<!--
